...
data.HGC.SAS=1; %1：使用流体网络法计算空气系统特性；0：使用定比例方法
%% 放气系统
...
%% 整机子程序
Power=[];z=[];SFC=[];W_in=[];T4_all=[];TEff=[];HPShaft_all=[];WfP3=[];
i=0;
for bianglian=-Power_output
    Load=bianglian;
    i=i+1;   
    if i==1
    ...
    HPShaft_all=[HPShaft_all,x(6)];
    WfP3=[WfP3,WholeEngine(i).B_Data.Wf/GasPth(i).GasOut_HPC.Pt];
end
%% 绘图
...
%%
% figure
% plot(1e7*(1:-0.05:0.05),WfP3)
GasPth.GasOut_Inlet;
GasPth.GasOut_HPC;
GasPth.GasOut_Burner;
GasPth.GasOut_HPT;
GasPth.GasOut_PT;
...
WholeEngine.HPTData;
WholeEngine.PTData;
WholeEngine.HP_Shaft;
WholeEngine.PT_Shaft;
%%
