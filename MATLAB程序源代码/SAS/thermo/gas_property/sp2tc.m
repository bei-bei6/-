...
CPAIR = [-3.628178988352e-5,-2.100192023298e-5,...
...
DPAIR = [5.093289883514e-8,2.582130349498e-8,1.154429475734e-8,...
...
APSTOC = [4.208565000000e1,4.414325000000e1,4.576019000000e1,...
...
BPSTOC = [2.361110000000e-2,1.800764547338e-2,1.459451810646e-2,...
...
CPSTOC = [-3.50184547338e-5,-2.10160905323e-5,-1.31151831369e-5,...
...
DPSTOC = [4.667454733830e-8,2.633635798468e-8,1.107921302316e-8,...
...
while ((abs(S - Sg) >= Stol && jj < Jmax) || (jj < 2))
    ...
    it = ITAB(Tindex) - 1;
    dl = Tg - TTTAB(it+1);
    phiea =((DPAIR(it+1)*dl + CPAIR(it+1))*dl + BPAIR(it+1))*dl + APAIR(it+1);
    if (fa > 0)
        phisp = ((DPSTOC(it+1)*dl + CPSTOC(it+1))*dl + BPSTOC(it+1))*dl + APSTOC(it+1);
    else
        phisp = 0;
    end
    ...
    Sg = (phig*0.5035576347-23.0258509)*rcas - 0.1841304 - rcas * log(P/14.696);
