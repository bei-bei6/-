function [dydx,extra]=RAP_Body_Body(x,y,data)
R=data.Cons.R;
...
w1=2*pi*(data.RESI/60);
w2=2*pi*(data.RESO/60);
Vg=sqrt(gamma*R*Ts);
...
if data.RESI==0 && data.RESO==0% stationary passages
    if Re<=2000 % 层流
        if (r_ratio<= 0)
    ...
    % f0=0.307*Re^-0.24;
    f0=0.25/(log10(data.roughness/(3.7*D)+5.74/Re^0.9))^2;
    ...
    elseif data.RESI==0 && data.RESO~=0% 外壁面旋转
        ...
        f_f0=RAP_f0Calc(Rew/Re,data.d1/data.d2);
        f=f_f0*f0;
    else %两壁面旋转
