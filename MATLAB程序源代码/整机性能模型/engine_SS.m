function [F,GasPth,WholeEngine]=engine_SS(x,data)
% fprintf('x=[%s]\n',num2str(x','%f  '));
W=x(1);%实际入口流量？
...
if data.HGC.SAS==1
    ...
    F=[HPC_NErr;HPT_NErr;PT_NErr;Core_NErr;HP_Ndot;PT_Ndot;P41_NErr];
else
    F=[HPC_NErr;HPT_NErr;PT_NErr;Core_NErr;HP_Ndot;PT_Ndot];
end
fprintf('NErr=[%s]\n',num2str(F','%f  '));
%%
...
assignin('base','PT_Shaft0',PT_Shaft);
end