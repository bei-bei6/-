function [F,GasPth,WholeEngine]=engine_DS_NoSAS_LIMITall2(x,data)
...
switch data.type
    case 1
        NErr1=(HP_Ndot-data.maxXdot_HPShaft)*divby(data.maxXdot_HPShaft);
        WfP3=B_Data.Wf/GasOut_HPC.Pt;
        NErr3=(WfP3-data.maxWfP3)*divby(data.maxWfP3);
%         if NErr3>NErr1
%             deoc=1;
%         end
        NErr=max(NErr1,NErr3);
    case 2
        NErr2=-(HP_Ndot-data.minXdot_HPShaft)*divby(data.minXdot_HPShaft);
        WfP3=B_Data.Wf/GasOut_HPC.Pt;
        NErr4=(WfP3-data.minWfP3)*divby(data.minWfP3);
        NErr=min(NErr2,NErr4);
end       
