function [GasOut,NErr,TrqOut,Data]=LPT(CoolFlow,GasIn,Rline,Nmech,data)
C_PSTD= data.Cons.C_PSTD;
C_TSTD=data.Cons.C_TSTD;

X_Ncor_r =data.PTdate.X_Ncor_r;%相对换算转速
X_Beta = data.PTdate.X_Beta;%beta
Y_Wcor =data.PTdate.Y_Wcor;%换算流量
Y_Eff =data.PTdate.Y_Eff;%换算效率
Y_PR =data.PTdate.Y_PR;%换算压比

scale_Wc = data.scale.PT.Wc;
scale_PR = data.scale.PT.PR;
scale_Eff = data.scale.PT.Eff;
scale_Nc = data.scale.PT.Nc;
%% 入口参数
WIn=GasIn.W;
PtIn=GasIn.Pt;
TtIn=GasIn.Tt;
...
%% Stator后截面参数计算
W_Stator=WIn+CoolFlow.PT_stator.W;
Pt_Stator=PtIn;
ht_Stator=(htIn*WIn+CoolFlow.PT_stator.ht*CoolFlow.PT_stator.W)*divby(W_Stator);
...
Correct_W=interp1g(data.Correction.LPT.Nc,data.Correction.LPT.W,Ncor_r);
Correct_Eff=interp1g(data.Correction.LPT.Nc,data.Correction.LPT.Eff,Ncor_r);
Correct_PR=interp1g(data.Correction.LPT.Nc,data.Correction.LPT.PR,Ncor_r);
...
Pwrout=(ht_Stator-ht_Rotor)*W_Rotor*data.LPS.Eff;%这里与高压涡轮不同
TrqOut = Pwrout*divby(Nmech);
%% rotor冷气掺混
W_Rotor_after=W_Rotor+CoolFlow.PT_rotor.W;
Pt_Rotor_after=Pt_Rotor;
ht_Rotor_after=(ht_Rotor*W_Rotor+CoolFlow.PT_rotor.ht*CoolFlow.PT_rotor.W)*divby(W_Rotor_after);
...
%% 输出参数
if data.Metal_open
    Q=data.Metal.Ah_LPT*((Tt_Rotor_after+TtIn)/2-data.Metal.T_LPT);
    ht_Rotor_after=ht_Rotor_after-Q/WIn;
    Tt_Rotor_after = hf_dp2t(ht_Rotor_after,FAR_Rotor_after,d,Pt_Rotor_after);
    dT_matel=Q*divby(data.Metal.mcp_LPT);
else
    dT_matel=0;
    data.Metal.T_LPT=0;
end
